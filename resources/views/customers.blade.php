                  ...
                  {{-- <span class="d-none d-sm-inline">
                    <a href="#" class="btn btn-white">
                      New view
                        ...
                        <a class="btn btn-primary d-none d-sm-inline-block mb-2" data-bs-toggle="modal"
...
<form method="GET" action="{{ route('customers') }}" class="mb-3 d-flex gap-2 align-items-center">
    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search..." class="form-control" />

    <select name="estado" onchange="this.form.submit()" class="form-select">
        <option value="">All</option>
        <option value="active" {{ request('estado') === 'active' ? 'selected' : '' }}>Active</option>
        <option value="inactive" {{ request('estado') === 'inactive' ? 'selected' : '' }}>Inactive</option>
    </select>
    ...
    <tr>
        ...
        <td><a  class="text-reset" tabindex="-1">{{  ucwords(strtolower($contacto['contactName']))  ?? '—' }}</a></td>
        <td>{{ $contacto['email'] ?? '—' }}</td>
        <td>
        {{ $contacto['phone'] ?? '—' }}
            {{-- <!-- @php
                $estado = strtolower($contacto['estado'] ?? 'pendiente');
                $badgeClass = match($estado) {
                    'pagado', 'paid' => 'bg-success',
                    'pendiente', 'pending' => 'bg-warning',
                    'vencido', 'overdue' => 'bg-danger',
                    default => 'bg-secondary',
                };
            @endphp
            <span class="badge {{ $badgeClass }} me-1"></span> {{ ucfirst($estado) }} --> --}}
        </td>
...
@endforeach
    ...
    <p class="m-0 text-muted">
        Showing <span>{{ $contactos->firstItem() }}</span> to <span>{{ $contactos->lastItem() }}</span> of <span>{{ $contactos->total() }}</span> entries
    </p>
